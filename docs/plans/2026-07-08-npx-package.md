...
**Goal:** Publish this local Codex config board as an npm package that can run with `npx @nolann-dev/cxcg`.
...
Rename the publish package to `@nolann-dev/cxcg`, expose `cxcg` and `cxconfig` binaries, build the web bundle, bundle the CLI with esbuild, and publish only the compiled CLI, UI assets, README, and package metadata.
...
Document `npx @nolann-dev/cxcg`, CLI flags, npm login/publish flow, and keep local development commands separate from consumer quick start.

### Task 4: Verification

Run `pnpm typecheck`, `pnpm test`, `pnpm build`, `npm pack --dry-run`, install/run the packed tarball through `npx --yes ./nolann-dev-cxcg-*.tgz --no-open --api-port <port>`, verify the served page responds, then publish with `npm publish --access public`.
